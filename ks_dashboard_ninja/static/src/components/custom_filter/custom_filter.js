...
export class CustomFilter extends Component{
    ...
    async loadFieldDefsAndLabels(filters_list) {
        ...
        for (const filter of filters_list) {
            ...
            if (typeof filter.field_name === "string") {
                promises.push(
                    this.fieldService.loadFieldInfo(filter.model, filter.field_name).then(({ fieldDef }) => {
                ...
                value =  value.flatMap(
                    item => {
                        if(item === "%UID") {
                            ...
                            return ["in", "not in"].includes(operator) ? user.activeCompanies.map(c => c.id) : user.activeCompany.id;
                        } else {
                            return item;
                        }
                    }
    ...
    async applyFilters() {
        ...
        for( let model of Object.keys(modelConnectors)){
            ...
            let label = await this.treeProcessor.getDomainTreeDescription(model, modelConnectors[model])
