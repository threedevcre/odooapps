...
export class ExplanationViewer extends Component {
    ...
    setup(){
        ...
        useBus(this.env.bus, 'DN: Pause All', ({ detail }) => {
            ...
            if(detail.itemId != this.props.item.id){
                this.state.isPlay = false
            }
        })


        useBus(this.env.bus, `ITEM-${this.props.itemId}: Fetch AI Summary`, this.fetchExplanation.bind(this))
    ...
    async fetchExplanation(){
        ...
        let args = [[config], [], this.props.item.ks_dashboard_id]
    ...
    async speakText(){
        ...
        this.env.bus.trigger('DN: Pause All', { itemId: this.props.item.id })
