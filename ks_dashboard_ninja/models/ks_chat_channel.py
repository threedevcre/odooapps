...
class ChatChannel(models.Model):
    ...
    def ks_chat_wizard_channel_id(self, item_id, dashboard_id, **kwargs):
        ...
        if not channel_id:
            users = self.env['res.users'].search([('group_ids', 'in', self.env.ref('base.group_user').ids)]).mapped('partner_id.id')
            ...
            Store(bus_channel=self.env.user).add(channel).bus_send()
            return channel.id
