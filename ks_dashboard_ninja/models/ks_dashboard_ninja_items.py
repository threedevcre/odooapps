...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_dashboard_ninja_board_id = fields.Many2one('ks_dashboard_ninja.board', string="Dashboard",
                                                  default=lambda self: self.env.context[
                                                      'ks_dashboard_id'] if 'ks_dashboard_id' in self.env.context
                                                  else False)
    ...
    ks_dashboard_item_type = fields.Selection([('ks_tile', 'Tile'),
                                               ...
                                               ], default=lambda self: self.env.context.get('ks_dashboard_item_type',
    ...
    ks_list_view_fields = fields.Many2many('ir.model.fields', 'ks_dn_list_field_rel', 'list_field_id', 'field_id',
                                           domain="[('model_id','=',ks_model_id),('ttype','!=','one2many'),"
                                                  "('ttype','!=','many2many'),('ttype','!=','binary'), ('ttype','!=','properties'), ('ttype','!=','json')]",
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if rec.ks_date_filter_field:
            if not rec.ks_date_filter_selection or rec.ks_date_filter_selection == "l_none":
                selected_start_date = self.env.context.get('ksDateFilterStartDate', False)
                selected_end_date = self.env.context.get('ksDateFilterEndDate', False)
                ks_is_def_custom_filter = self.env.context.get('ksIsDefultCustomDateFilter', False)
                ks_timezone = self.env.context.get('tz') or self.env.user.tz
                ...
                if self.env.context.get('ksDateFilterSelection', False) and self.env.context['ksDateFilterSelection'] not in [
                    'l_none', 'l_custom']:
                    ks_date_data = ks_get_date(self.env.context.get('ksDateFilterSelection'), self,
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period and rec.ks_year_period != 0 and rec.ks_dashboard_item_type:
                        ...
                        ks_date_domain = ['&', (date_field_name, ">=",
                                                datetime.strftime(selected_start_date,
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)),
                                          (date_field_name, "<=",
                                           datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT))]

                        for p in range(1, abs_year_period + 1):
                            ks_date_domain.insert(0, '|')
                            ks_date_domain.extend(['&', (date_field_name, ">=", datetime.strftime(
                                selected_start_date - relativedelta.relativedelta(years=p) * sign_yp,
                                DEFAULT_SERVER_DATETIME_FORMAT)),
                                                   (date_field_name, "<=", datetime.strftime(
                                                       selected_end_date - relativedelta.relativedelta(years=p)
                                                       * sign_yp, DEFAULT_SERVER_DATETIME_FORMAT))])
                    else:
                        selected_start_date = datetime.strftime(selected_start_date,
                                                                       DEFAULT_SERVER_DATETIME_FORMAT)
                        selected_end_date = datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT)
                        ks_date_domain = [(rec.ks_date_filter_field.name, ">=", selected_start_date),
                                          (rec.ks_date_filter_field.name, "<=", selected_end_date)]
                elif selected_start_date and not selected_end_date:
                    selected_start_date = datetime.strftime(selected_start_date, DEFAULT_SERVER_DATETIME_FORMAT)
                    ks_date_domain = [(rec.ks_date_filter_field.name, ">=", selected_start_date)]
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                        ...
                        if rec.env.context.get('current_id', False):
                            ks_item_id = rec.env.context['current_id']
                        else:
                            ks_item_id = rec.id

                        if rec.ks_date_filter_selection == "l_none":
                            selected_start_date = rec.env.context.get('ksDateFilterStartDate', False)
                            selected_end_date = rec.env.context.get('ksDateFilterEndDate', False)
    ...
    def _get_list_item_config(self, item_domain1):
        self.ensure_one()
        if self.ks_record_data_limit_visibility and self.ks_record_data_limit > 0:
            pagination_limit = min(self.ks_record_data_limit, self.ks_pagination_limit)
        else:
            pagination_limit = self.ks_pagination_limit
        return {
            'ks_list_view_data': self._ksGetListViewData(item_domain1),
            'ks_list_view_type': self.ks_list_view_type,
            'ks_pagination_limit': pagination_limit,
    ...
    def _get_graph_item_config(self, item_domain1):
        ...
        if self.ks_record_data_limit_visibility and self.ks_record_data_limit > 0:
            ...
            pagination_limit = self.ks_pagination_limit
        return {
            'ks_pagination_limit': pagination_limit, # in case of drill down list view
    ...
    def _ksGetListViewData(self, domain=[]):
        ...
        if self.ks_record_data_limit_visibility and self.ks_record_data_limit > 0:
            ks_list_view_data['total'] = self.ks_record_data_limit
    ...
    def get_list_view_record(self, orderid, sort_order, ks_chart_domain, ksoffset=0,
        ...
        if ks_limit:
            ...
            if ks_limit and ks_limit < self.ks_pagination_limit:
                ...
                limit = self.ks_pagination_limit
        if self.env.context.get('export_all', False):
            limit = ks_limit
            offset = 0
        ...
        if self.ks_list_view_type == "ungrouped":
        ...
        elif self.ks_list_view_type == "grouped" and self.ks_list_view_group_fields and self.ks_chart_relation_groupby:
            ks_list_fields = []
            ...
            if self.ks_chart_groupby_type == 'relational_type':
                ...
                try:
                    domain = Domain(ks_chart_domain)
                    if self.ks_chart_relation_groupby.name in ks_list_fields:
                        ks_list_fields.remove(self.ks_chart_relation_groupby.name)
                    aggregate = [ f'{field}:sum' for field in ks_list_fields]
                    ks_list_view_records = self.env[self.ks_model_name]. \
                    web_read_group(
                        domain, [self.ks_chart_relation_groupby.name], aggregate, order=orderby, offset=ksoffset,
                        limit=limit
                    )
                    ks_list_fields.insert(0, self.ks_chart_relation_groupby.name)

                    for record in ks_list_view_records.get('groups', []):
                        cleaned_record = {}
                        for key, value in record.items():
                            if ':' in key:
                                field_name = key.split(':', 1)[0]  # keep only part before ':'
                                cleaned_record[field_name] = value
                            else:
                                cleaned_record[key] = value

                        record.clear()
                        record.update(cleaned_record)
                except Exception as e:
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields) \
                        ...
                        data_row = {'groupById': res[self.ks_chart_relation_groupby.name] and res[self.ks_chart_relation_groupby.name][0], 'data': {},
                                    'domain': json.dumps(res['__extra_domain']), 'ks_column_type': []}
                ...
                try:
                    domain = Domain(ks_chart_domain)
                    if self.ks_chart_relation_groupby.name in ks_list_field:
                        ks_list_field.remove(self.ks_chart_relation_groupby.name)
                    aggregate = [f'{field}:sum' for field in ks_list_field]
                    ks_list_view_records = self.env[self.ks_model_name]. \
                        web_read_group(
                        domain, [self.ks_chart_relation_groupby.name + ':' + ks_chart_date_groupby], aggregate, order=orderby, offset=ksoffset,
                        limit=limit
                    )
                    ...
                    for record in ks_list_view_records.get('groups', []):
                        cleaned_record = {}
                        for key, value in record.items():
                            if ':' in key and key != self.ks_chart_relation_groupby.name + ':' + ks_chart_date_groupby:
                                field_name = key.split(':', 1)[0]  # keep only part before ':'
                                cleaned_record[field_name] = value
                            else:
                                cleaned_record[key] = value
                ...
                if all(list_fields in res for res in ks_list_view_records for list_fields in
                    ...
                    for res in ks_list_view_records:
                        counter = 0
                        data_row = {'id': 0, 'data': {}, 'domain': json.dumps(res['__extra_domain']), 'ks_column_type': []}
                        for field_rec in ks_list_fields:
                            if isinstance(res[field_rec], tuple):
                                data_row['data'][field_rec] = {'value': res[field_rec][1] if len(res[field_rec]) > 1 else res[field_rec][0] }
                            else:
                                data_row['data'][field_rec] = { 'value': res[field_rec] }
                        ks_list_view_data['records'].append(data_row)
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period_2 and rec.ks_year_period_2 != 0:
                        ...
                        if rec.ks_date_filter_field_2:
                            selected_start_date = datetime.strftime(selected_start_date,
                                                                           DEFAULT_SERVER_DATETIME_FORMAT)
                            selected_end_date = datetime.strftime(selected_end_date,
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)
            ...
            if self.ks_fill_temporal and ks_chart_date_groupby not in ['minute', 'hour']:
                ks_chart_records = self.env[ks_model_name].with_context(fill_temporal=True) \
                    .read_group(ks_chart_domain,
                                ...
                                orderby=False, limit=limit, lazy=False)
            else:
    ...
    def ks_fetch_drill_down_data(self, domain, sequence):
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            if action_line.ks_sort_by_order and orderby:
    ...
    def get_sorted_month(self, display_format, ftype='date'):
        ...
        tz_convert = self.env.context.get('tz')
    ...
    def format_label(self, value, ftype, display_format, tz_convert, locale):
        ...
        if ftype == 'datetime':
            if tz_convert:
                value = pytz.timezone(self.env.context['tz']).localize(value)
                tzinfo = value.tzinfo
