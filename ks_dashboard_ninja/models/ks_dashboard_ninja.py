...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_date_filter_selection = fields.Selection([
        ('l_none', 'All Time'), ('l_day', 'Today'), ('t_week', 'This Week'), ('t_month', 'This Month'),
        ('t_quarter', 'This Quarter'), ('t_year', 'This Year'), ('td_week', 'Week to Date'),
        ('td_month', 'Month to Date'), ('td_quarter', 'Quarter to Date'), ('td_year', 'Year to Date'),
        ('n_day', 'Next Day'), ('n_week', 'Next Week'), ('n_month', 'Next Month'), ('n_quarter', 'Next Quarter'),
        ('n_year', 'Next Year'), ('ls_day', 'Last Day'), ('ls_week', 'Last Week'), ('ls_month', 'Last Month'),
        ('ls_quarter', 'Last Quarter'), ('ls_year', 'Last Year'), ('l_week', 'Last 7 days'), ('l_month', 'Last 30 days'),
        ('l_quarter', 'Last 90 days'), ('l_year', 'Last 365 days'), ('ls_past_until_now', 'Past Till Now'),
        ('ls_pastwithout_now', ' Past Excluding Today'), ('n_future_starting_now', 'Future Starting Now'),
        ('n_futurestarting_tomorrow', 'Future Starting Tomorrow'), ('l_custom', 'Custom Filter'),
    ...
    ks_set_interval = fields.Selection([
                    ...
                    ('120000', '2 minute'), ('300000', '5 minute'), ('600000', '10 minute'), ],
    ...
    gs_layout_ids = fields.One2many('ks_dashboard_ninja.grid_stack_layouts', 'dn_dashboard_id', string='Dashboard Layouts')
    ...
    def create(self, vals):
        ...
        for record in records:
            ...
            if record.ks_dashboard_default_template and record.ks_dashboard_default_template.ks_item_count:
                ...
                self.gs_layout_ids.create({
                    'dn_dashboard_id': self.id, 'name': self.name,
                    'is_default': True, 'is_main_layout': True, 'grid_stack_config': ks_gridstack_config
                })
        return records
        ...
        for rec in self:
            ...
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board', False) and self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    if self.env.ref('ks_dashboard_ninja.board_menu_root', False):
                        self.env.ref('ks_dashboard_ninja.board_menu_root').group_ids = vals['ks_dashboard_group_access']
                else:
                    rec.ks_dashboard_menu_id.sudo().group_ids = vals['ks_dashboard_group_access']
            ...
            if not rec.ks_dashboard_group_access.ids:
                menu_ids.group_ids = False
            else:
                menu_ids.group_ids = list(set(menu_ids.group_ids.ids + [admin_group_id]))
            ...
            if 'name' in vals:
                ...
                main_grid_layout = rec.gs_layout_ids.filtered(lambda layout: layout.is_main_layout)
                if main_grid_layout.name != rec.name:
                    main_grid_layout.name = rec.name


        return record

    # TODO : Should use ondelete = 'cascade' rather than unlinking separately
    def unlink(self):
            ...
            if (rec.name == "My Dashboard" and rec.ks_dashboard_state == 'Locked') or(rec.name in ['Template1 Dashboard','Template2 Dashboard','Template3 Dashboard'] and rec.ks_dashboard_top_menu_id.name == 'My Dashboard'):
                ...
                if not rec.ks_dashboard_menu_id:
                    ...
                    rec.ks_dashboard_menu_id = self.env['ir.ui.menu'].sudo().create({
                        ...
                        'group_ids': rec.ks_dashboard_group_access.ids if rec.ks_dashboard_group_access else False,
    ...
    def read_items_data(self, item_basic_data_fields):
        return self.ks_dashboard_items_ids.read(item_basic_data_fields)

    def read_gs_layouts(self):
        if not len(self.gs_layout_ids):
            self.gs_layout_ids.create({ 'dn_dashboard_id': self.id, 'name': self.name, 'is_default': True, 'is_main_layout': True})

        return self.gs_layout_ids.read(['name', 'grid_stack_config', 'is_default', 'is_main_layout'])

    def set_default_layout(self, layout):
        default_layouts = self.gs_layout_ids.filtered(lambda l: l.is_default == True and l.id != layout.id)
        default_layouts.write({'is_default': False})
        layout.write({'is_default': True})

    def layout_changes(self, layout_id, vals, set_default=False):
        """
        Set the default layout for the dashboard.
        :param layout_id: ID of the layout to be set as default.
        :param vals: Dictionary of values to be updated in the layout.
        :param set_default: Boolean to indicate if the layout should be set as default.
        """

        target_layout = self.gs_layout_ids.filtered(lambda l: l.id == layout_id)
        if not target_layout:
            target_layout = self.gs_layout_ids.create(vals)
        else:
            target_layout.write(vals)

        if set_default:
            self.set_default_layout(target_layout)

        return self.read_gs_layouts()

    # @api.model
    def ks_fetch_dashboard_configs(self, ks_item_domain=[]):
        """
        ...
        dashboard_fields = ['name', 'ks_set_interval', 'ks_data_formatting', 'ks_ai_explain_dash', 'is_bookmarked']
        item_basic_data_fields = ['id', 'ks_dashboard_item_type', 'ks_model_name_2', 'ks_model_name']
        configs = {
            'dashboard_data': self.read(dashboard_fields)[0],
            'items_basic_data': self.read_items_data(item_basic_data_fields),
            'gs_layouts_info': {
                'gs_layouts': self.read_gs_layouts(),
                'multi_layouts': self.multi_layouts
            },
        }
        dashboard_data = configs['dashboard_data']

        dashboard_data.update({
            'item_basic_data_fields': item_basic_data_fields,
            'ks_company_id': self.env.context.get('allowed_company_ids')[0],
            'ks_dashboard_manager': has_group_ks_dashboard_manager, # TODO: do it another way
            'ks_dashboard_list': self.search_read([], ['id', 'name']),  # TODO: check if the dashboards gets inactive
            'dateFilterData': {
                'config': {
                    'startDate': fields.Datetime.context_timestamp(self, self.ks_dashboard_start_date) if self.ks_dashboard_end_date else False,
                    'endDate': fields.Datetime.context_timestamp(self, self.ks_dashboard_end_date) if self.ks_dashboard_end_date else False,
                    'dateSelection': self.ks_date_filter_selection ,
                },
                'ks_default_end_time': self.ks_default_end_time
            },
            'ks_child_boards': False,
            'ks_selected_board_id': False,
            'ks_dashboard_pre_domain_filter': self.ks_prepare_dashboard_pre_domain(),
            'ks_dashboard_custom_domain_filter': self.ks_prepare_dashboard_custom_domain(),
            'ks_dashboard_favourite_filter': self.ks_prepare_dashboard_favourite_filter(),
            'ks_dashboard_items_ids': self.ks_dashboard_items_ids.ids, # TODO: can be done by other ways ??
        })
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        item = {
            ...
            'ks_to_do_data': rec.get_todo_data(),
    ...
    def ks_set_date(self, ks_dashboard_id):
        ...
        if self.env.context.get('ksDateFilterSelection', False):
            ks_date_filter_selection = self.env.context['ksDateFilterSelection']
            if ks_date_filter_selection == 'l_custom':
                ks_start_dt_parse = parse(self.env.context['ksDateFilterStartDate'])
                ks_end_dt_parse = parse(self.env.context['ksDateFilterEndDate'])
                self = self.with_context(
                    ksDateFilterStartDate=datetime.strptime(ks_start_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                                                                   "%Y-%m-%d %H:%M:%S"))
                self = self.with_context(
                    ksDateFilterEndDate=datetime.strptime(ks_end_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                                                                 "%Y-%m-%d %H:%M:%S"))
    ...
    def fetch_dashboard_overview(self, **kwargs):
        ...
        dashboard_condition = ('ks_dashboard_ninja_board_id', 'in', ids)
        map_count = ks_board_item_model.search_count([('ks_dashboard_item_type', '=', 'ks_map_view'), dashboard_condition])
        list_view_count = ks_board_item_model.search_count([('ks_dashboard_item_type', '=', 'ks_list_view'), dashboard_condition])
    ...
    def ks_view_items_view(self):
        ...
        return {
            ...
            'help': _('''<p class="o_view_nocontent_smiling_face">You can find all items related to Dashboard Here.</p>'''),
    ...
    # FIXME: move the method to item model file
    # fetching Item info (Divided to make function inherit easily)
    def ks_export_item_data(self, rec):
        ...
        for res in rec.ks_goal_lines:
            goal_line = {
                'ks_goal_date': datetime.strftime(res.ks_goal_date, "%Y-%m-%d"),
        ...
        val = str(rec.id)
        keys_data = {}
        # TODO: fix below code - why search?
        selecred_rec = self.env['ks_dashboard_ninja.grid_stack_layouts'].search(
            [['id', 'in', rec.ks_dashboard_ninja_board_id.gs_layout_ids.ids], ['is_main_layout', '=', True]], limit=1)
        ...
        elif selecred_rec and selecred_rec.grid_stack_config:
            keys_data = selecred_rec.grid_stack_config
        elif self.env.context.get('gridstack_config', False):
            keys_data = self.env.context.get('gridstack_config', False)
        else:
            if rec.grid_corners:
    ...
    # TODO: improve the method logic and way of writing
    def ks_import_item(self, item_fields, **kwargs):
        try:
        ...
        item['ks_dashboard_ninja_board_id'] = self.id
        item['ks_company_id'] = False

        try:
            dashboard_item = self.ks_create_item(item)
        except Exception:
            raise ValidationError(_("Something went wrong."))

        return dashboard_item.read(item_fields)[0]
    ...
    def ks_dashboard_export(self, ks_dashboard_ids, **kwargs):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            selecred_rec = self.env['ks_dashboard_ninja.grid_stack_layouts'].search(
                [['id', 'in', dash.ks_child_dashboard_ids.ids], ['is_main_layout', '=', True]], limit=1)
            # TODO: why browse - same id we have searched above ?? logic suicides
            ks_dashboard_rec = self.browse(ks_dashboard_id)
            if selecred_rec:
                grid_conf = selecred_rec.grid_stack_config
            else:
                grid_conf = {}
            ...
            if selecred_rec:
                ...
                dashboard_data['grid_stack_config'] = selecred_rec.grid_stack_config
    ...
    def ks_import_dashboard(self, file, menu_id):
        ...
        for data in ks_dashboard_data:
            ...
            ks_gridstack_config = {}
            if data.get('grid_stack_config', False):
    ...
    def ks_create_item(self, item):
        ...
        if ks_goal_lines and len(ks_goal_lines) != 0:
            for line in ks_goal_lines:
                line['ks_goal_date'] = datetime.strptime(line['ks_goal_date'].split(" ")[0],
                                                                  '%Y-%m-%d')
    ...
    # FIXME: USE orm Read and remove extra search
    ...
    def ks_prepare_dashboard_favourite_filter(self):
        data = {}
        for rec in self.ks_dashboard_favourite_filters_ids.filtered(
                lambda r: r.user_id.id == self.env.user.id or not r.user_id
        ):
            data[rec.id] = {
                'id': rec.id,
