...
class KSduplicateDashboardWizard(models.TransientModel):
    ...
    def DuplicateDashBoard(self):
        # FIXME: why this function is made for calling action ??? can be directly open from js
        '''this function returns acion id of ks.dashboard.duplicate.wizard'''
        action = self.env['ir.actions.act_window']._for_xml_id('ks_dashboard_ninja.ks_duplicate_dashboard_wizard')
