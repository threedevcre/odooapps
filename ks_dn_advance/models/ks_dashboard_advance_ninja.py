...
class KsDashboardNinjaAdvance(models.Model):
    ...
    # @api.model
    def ks_fetch_dashboard_configs(self, ks_item_domain=False):
        configs = super(KsDashboardNinjaAdvance, self).ks_fetch_dashboard_configs(ks_item_domain)
        configs['dashboard_data']['ks_croessel_speed'] = self.ks_croessel_speed
        ...
        if ks_mail_validate:
            try:
                ...
                if ks_mail_template:
                    try:
                        email_values = {
                            'email_to': self.ks_mail_to_partner.email,
                            'email_cc': self.ks_mail_email_cc_value,
                        }
                        ks_mail_template.with_context(ks_ctx).send_mail(self.id, force_send=True, email_values=email_values)
