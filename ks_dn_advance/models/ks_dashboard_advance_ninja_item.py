...
class KsDashboardNinjaItemAdvance(models.Model):
    ...
    def ks_format_query_result(self, ks_query_result):
        ...
        if not self.env.context.get('export_all', False):
        ...
        for rec in self:
            if rec.ks_is_date_ranges:
                ...
                if self.env.context.get('ksDateFilterSelection', False):
                    ksDateFilterSelection = self.env.context.get('ksDateFilterSelection', False)

                    if ksDateFilterSelection == 'l_custom':
                        ks_timezone = self.env.context.get('tz') or self.env.user.tz
                        selected_start_date = self.env.context['ksDateFilterStartDate']
                        selected_end_date = self.env.context['ksDateFilterEndDate']
    ...
    def ks_get_list_data_orderby_extend(self, order_id, sort_order, offset, domain={}):
        ...
        ks_field = self.env.context.get('field', False)
        ks_domain = []
        ...
        if self.env.context.get('ksDateFilterSelection', False):
            ...
            if ks_date_filter_selection == 'l_custom':
                self = self.with_context(
                    ksDateFilterStartDate = parse(self.env.context['ksDateFilterStartDate']))
                self = self.with_context(
                    ksDateFilterEndDate = parse(self.env.context['ksDateFilterEndDate']))
        ...
        if self.ks_record_data_limit_visibility and self.ks_record_data_limit > 0:
            list_view_data['total'] = self.ks_record_data_limit
